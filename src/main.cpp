...
void drawPilihProduk() {
  tft.fillScreen(ILI9341_BLACK);
  tft.setTextSize(2);
  tft.setTextColor(ILI9341_YELLOW);
  tft.setCursor(50, 10);
  tft.print("Pilih Produk");

  // 4 tombol grid 2x2
  int bx[] = { 10, 130, 10, 130 };
  int by[] = { 45, 45, 175, 175 };

  for (int i = 0; i < TOTAL_PRODUK; i++) {
    tft.fillRoundRect(bx[i], by[i], 100, 115, 8, 0x2945);
    tft.drawRoundRect(bx[i], by[i], 100, 115, 8, ILI9341_CYAN);

    tft.setTextSize(1);
    tft.setTextColor(ILI9341_WHITE);
    tft.setCursor(bx[i] + 8, by[i] + 18);
    tft.print(produk[i].nama);

    tft.setTextColor(ILI9341_GREEN);
    tft.setCursor(bx[i] + 8, by[i] + 55);
    tft.print("Rp");
    tft.setCursor(bx[i] + 8, by[i] + 68);
    tft.print(produk[i].harga);
  }
}
...
void loop() {
  ...
  switch (currentState) {
    ...
    case STATE_IDLE: {
      ...
      for (int i = 0; i < TOTAL_PRODUK; i++) {
        if (p.x >= bx[i] && p.x <= bx[i] + 100 &&
            p.y >= by[i] && p.y <= by[i] + 115) {
          produkDipilih = i;
          transAmount   = produk[i].harga;
    ...
    case STATE_LOADING_DOWNLOAD_QR: {
      ...
      tft.setTextColor(ILI9341_WHITE);
      tft.setCursor(20, 36);
      tft.print(produk[produkDipilih].nama);
      tft.setTextColor(ILI9341_GREEN);
      tft.setCursor(150, 36);
      tft.print("Rp"); tft.print(produk[produkDipilih].harga);

      const int QR_SIZE = 200;
      const int QR_X    = (240 - QR_SIZE) / 2;
      const int QR_Y    = 55;
