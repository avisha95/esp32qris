...
Mayar::Mayar(const String& apiKey)
  : _apiKey(apiKey), _timeout(10), _insecure(true),
    _autoReconnect(true), _debug(false) {}
...
bool Mayar::begin(const char* ssid, const char* password, int timeoutSec) {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - start < (unsigned long)timeoutSec * 1000UL) {
    delay(500);
    if (_debug) Serial.print(".");
  }
...
QRISResponse Mayar::_request(const String& method, const String& endpoint, const String& payload) {
  ...
  if (_insecure) client.setInsecure();
  ...
  if (_debug) {
    if (result.success) {
      if (result.balance > 0) {
        ...
        Serial.print(" (active: Rp ");           Serial.print(result.balance_active);
        Serial.println(")");
      }
      if (result.url.length() > 0 && result.url != "null") {
        Serial.println("[Mayar] QRIS created successfully.");
        Serial.print("[Mayar] URL: "); Serial.println(result.url);
      }
    } else {
...
bool Mayar::waitForPayment(long initialBalance,
  ...
  while (millis() - start < timeoutMs) {
    ...
    if (bal.success) {
      ...
      if (_debug) {
        Serial.print("[Mayar] Polling... balance: Rp "); Serial.print(current);
        Serial.print(" ("); Serial.print(elapsed / 1000); Serial.println("s)");
      }
      if (current > initialBalance) {
        if (_debug) { Serial.print("[Mayar] Payment received! Rp "); Serial.println(current); }
...
TFTKeypad::TFTKeypad(Adafruit_ILI9341& display, Adafruit_FT6206& touch)
  : _tft(&display), _ctp(&touch), _value(""),
    _lastTouch(0), _wasReleased(true) {}
...
void TFTKeypad::process() {
  ...
  if (now - _lastTouch < (unsigned long)DEBOUNCE_MS) { delay(3); return; }
    ...
    for (int c = 0; c < 3; c++) {
      ...
      if (k >= '0' && k <= '9') {
        ...
        if (_delCb) {
          ...
          if (_value.length()) { _value.remove(_value.length() - 1); updateDisplay(); }
        }
      } else if (k == 'O') {
        if (_okCb) _okCb();
      }
...
void TFTKeypad::drawKey(int row, int col, bool pressed) {
  ...
  _tft->setCursor(x + (KEY_W - bw) / 2 - bx,
                  y + (KEY_H - bh) / 2 - by + 1);
  _tft->print(str);
}
...
bool TFTImageDisplay::show(const String& url,
...
bool TFTImageDisplay::downloadAndDecode(const String& url, int timeoutSec,
  ...
  for (int retry = 0; retry < 3; retry++) {
    if (retry == 0) {
      ...
      Serial.print("[Image] Retrying ("); Serial.print(retry + 1); Serial.println("/3)...");
      delay(2000UL * retry);
    ...
    while (clientPtr->connected() && millis() - t < 5000) {
      if (clientPtr->available()) {
        ...
        String hdrLow = hdr; hdrLow.toLowerCase();
    ...
    size_t bufSize = (contentLength > 0 && contentLength <= (int)MAX_IMG_SIZE)
                     ? (size_t)contentLength : MAX_IMG_SIZE;
    ...
    // ── Decode PNG ────────────────────────────────────────────────────────
    int rc = _png.openRAM(pngBuffer, (int)idx, TFTImageDisplay::_pngDrawCallback);
    if (rc == PNG_SUCCESS) {
    ...
    Serial.print("[Image] PNG decode failed (rc="); Serial.print(rc); Serial.println("), retrying...");
    free(pngBuffer);
  }
